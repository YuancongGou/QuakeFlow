########################################
# download (softlink) ncedc data locally
########################################
...
#                 )
                ...
                ncedc_filename = f"{station['station']}.{station['network']}.{station['instrument']}{comp}.{station['location']}.D.{year}.{julday}"
                bulk.append(
                    f"/data/ncedc/{station['network']}/{year}/{year}.{julday}/{ncedc_filename}"
                )
                ms_id.append(mseed_name)
        ...
        print(f"Soft-linking from ncedc : {starttime.isoformat()} - {endtime.isoformat()} {len(bulk)} traces")

        retry = 0
        while retry < max_retry:
            try:
                for i,ms_file_path in enumerate(bulk):
                    if not os.path.exists(ms_file_path):
                        print(ms_file_path + ' does not exist, skipped.')
                        continue
                    os.system('ln -s ' + ms_file_path + ' ' + f"{root_path}/{waveform_dir}/{ms_id[i]}")
                break
